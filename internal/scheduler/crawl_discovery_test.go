...
type fakeDisc struct{ got []string }

func (f *fakeDisc) EnqueueLinks(_ context.Context, _ model.Site, _ model.URL, links []string) (int, error) {
	f.got = append(f.got, links...)
	return len(links), nil
}

func TestCrawlOneEnqueuesDiscoveredLinks(t *testing.T) {
	t.Parallel()
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, _ *http.Request) {
		_, _ = w.Write([]byte(`<html><head><title>T</title></head><body><a href="/found">found</a></body></html>`))
	})
	mux.HandleFunc("/found", func(w http.ResponseWriter, _ *http.Request) {
		_, _ = w.Write([]byte(`<html><head><title>Found</title></head><body></body></html>`))
	})
	srv := httptest.NewServer(mux)
	defer srv.Close()

	st, err := store.Open(ctx, filepath.Join(t.TempDir(), "disc.db"))
	if err != nil {
		t.Fatalf("Open: %v", err)
	}
	...
	if !found {
		t.Errorf("expected fakeDisc to receive %q; got links: %v", srv.URL+"/found", fd.got)
	}
}
